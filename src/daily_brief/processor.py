...
- tw_market：限 5 則；判斷標準：新聞主詞必須是整體市場、加權指數、政策單位、央行、外資整體買賣超或匯率，**絕對不放任何涉及特定公司、個股或特定產業族群的新聞**；若有疑慮一律歸入 hot_stocks 或 industry，tw_market 寧缺毋濫
- us_macro：限 5 則；內容限定為 Fed 決議、Powell 發言、利率走勢、油價、美元指數、非農、CPI 等無產業歸屬的總體經濟訊號；不放個股或產業新聞
- industry：各產業欄聚焦台灣上市／上櫃公司新聞；科技巨頭(美)與半導體設備兩欄例外，可放對台灣市場有直接影響的國際廠商新聞；每產業最多 4 則；**同一篇新聞的 link 只能出現在一個產業欄**，跨產業的文章歸入最相關的那一欄，其他欄不重複放
- hot_stocks：今日新聞中被具體討論的公司，以台股為主；若有重大事件的美股（如 NVDA 財報、Apple 供應鏈）亦可納入；3-8 家
...
def _deduplicate_result(result: dict) -> dict:
    ...
    for stock in result.get("hot_stocks", []):
        ...
        if isinstance(rn, list):
            stock["related_news"] = _clean(rn)
        elif isinstance(rn, dict) and rn.get("link") in seen:
