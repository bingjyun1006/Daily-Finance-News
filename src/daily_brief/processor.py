...
- tw_market：限 8 則最有信號的新聞；判斷標準：新聞主詞必須是整體市場、加權指數、政策單位、央行、外資整體動向或產業面趨勢，**絕對不放任何涉及特定公司或個股的新聞**；若有疑慮，一律歸入 hot_stocks 或 industry，tw_market 寧缺毋濫
- 全域去重：產出前必須自行逐一核對每則新聞的 link，同一個 link 只能出現在整份 JSON 的一個區塊；若發現重複，保留較精確的區塊（如 hot_stocks > tw_market），另一個刪除
