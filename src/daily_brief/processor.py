...
def process_with_claude(market_data: dict, articles: list[dict],
    ...
    print(f"[Claude-Main] input={response.usage.input_tokens} cache_read={cache_read} cache_write={cache_write} output={response.usage.output_tokens}")
    ...
    try:
        ...
        print("[WARN] Claude-Main returned invalid JSON, using fallback structure")
...
def _process_industry_with_claude(client: anthropic.Anthropic, articles: list[dict], date_prefix: str) -> dict:
    ...
    try:
        ...
        cache_read = getattr(response.usage, "cache_read_input_tokens", 0)
        cache_write = getattr(response.usage, "cache_creation_input_tokens", 0)
        print(f"[Claude-Industry] input={response.usage.input_tokens} cache_read={cache_read} cache_write={cache_write} output={response.usage.output_tokens}")
