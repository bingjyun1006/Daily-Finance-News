...
- industry：收錄對台股該產業投資人有參考價值的台灣相關新聞，範疇包含：台灣個別公司動態（業績、訂單、擴產、新品、法說、外資調評）、產業整體趨勢、法規政策影響、券商／法人研究報告、專家產業評論；每產業選出最有參考價值的前 3 則，不足 3 篇無妨；不具參考價值（廣告稿、情緒標題、無具體資訊）的新聞不列，寧少勿濫；**同一篇新聞的 link 只能出現在一個台股產業欄，且不可與 us_industry 重複**；無相關新聞給空陣列 []
- us_industry：美國及國際公司（NVDA/AMD/ASML/Samsung/Micron 等）依產業歸類；每產業選出最有參考價值的前 3 則，不足 3 篇無妨；不具參考價值的新聞不列，寧少勿濫；**同一篇新聞的 link 只能出現在一個美股產業欄，且不可與 industry 重複**；無相關新聞給空陣列 []
...
def _fallback_structure(articles: list[dict]) -> dict:
    ...
    return {
        ...
        "us_industry": {k: [] for k in ["半導體(美)", "半導體設備(美)", "記憶體(美)", "Big Tech(美)", "軟體/AI(美)", "金融(美)", "生技(美)"]},
        "tw_movers_news": [],
        "us_movers_news": [],
        "market_radar": [],
    }
