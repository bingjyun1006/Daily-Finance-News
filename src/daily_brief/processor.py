...
必須以合法 JSON 格式回應（不要加 markdown code block），結構如下：
{
  ...
  "market_narrative": "3-5句白話文敘事，串聯今日台股、美股、總經的主要驅動力，說明市場今天在看什麼、為何如此波動，並點出今日值得特別關注的議題、產業或個股（不要條列，用段落文字）",
...
- market_narrative：3-5 句白話文，串聯當日台股、美股、總經的主要驅動力；說明今天市場的核心敘事是什麼、為何如此表現；最後點出 1-2 個今日特別值得關注的議題、產業或公司及原因
